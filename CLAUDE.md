...
### 17. Screenshot Tray + Voice Annotation (macOS, in progress)

Lets the user attach visual context to a dictation: capture one or more screenshots, optionally draw a drag-arrow on a frozen snapshot, and have the images land in the focused chat input (Cursor / Claude Code) right above the dictated text — one chat message, all visuals in order. Inspired by Google DeepMind's Magic Pointer (point/gesture + speak). The arrow + pixels carry the "this/that" pointing; voice is only for what cannot be pointed at (colour, font, behaviour). No NLP intent layer.

**Manager**: `src/helpers/screenshotManager.js` — `ScreenshotManager` holds an in-memory tray (`NativeImage[]`, NOT the clipboard, which holds only one item). Key methods: `captureRegionToTray()` (native `screencapture -i`, returns false on Esc/cancel), `captureCursorDisplay()` (native `screencapture -x -D<id>`, the freeze snapshot for phase 2), `addDataUrl()`, `takeAll()`, `getTrayCount()`, `clearTray()`, `getScreenPermissionStatus()`. Instantiated in `main.js` and passed into `IPCHandlers`.

**Phase 1 (built): tray + region capture + paste glue**
- Hotkey slot `"screenshot"` (default `CommandOrControl+Shift+2`, persisted via `environment.js` `getScreenshotKey/saveScreenshotKey` → `SCREENSHOT_KEY`). Registered in `main.js` next to the agent/meeting slots; rebindable via `register-screenshot-hotkey` IPC. The callback calls `textEditMonitor.captureTargetPid()` (so paste targets the right app even without dictation) then `captureRegionToTray()`.
- Paste path: `clipboard.js` `pasteImage(image, options)` mirrors `pasteText` but writes an image to the clipboard. The native paste (`pasteMacOS`/`pasteWindows`/`pasteLinux`) only simulates Cmd+V and is content-agnostic, so an image pastes exactly like text. No change to `macos-fast-paste.swift`.
- Flush point: the `paste-text` IPC handler in `ipcHandlers.js`. When dictation completes and the renderer calls `paste-text`, if the tray is non-empty it first pastes each screenshot one-by-one (300ms apart, chat inputs stack them as separate attachments), then the dictated text. The clipboard is saved once and restored once around the whole batch. Empty dictation + non-empty tray still pastes the images.
- Helper IPC: `screenshot-tray-count`, `clear-screenshot-tray`, `get-screen-permission-status`. Renderer event `screenshot-tray-updated` (count) on each capture. Preload methods: `registerScreenshotHotkey`, `getScreenshotTrayCount`, `clearScreenshotTray`, `getScreenPermissionStatus`, `onScreenshotTrayUpdated`.
- **Permission**: needs macOS Screen Recording (first `screencapture` triggers the OS prompt; a restart may be required after granting). This is a third permission on top of Microphone + Accessibility.
- **Design note**: sequential per-image paste is used instead of a composited montage because there is no image-compositing lib (sharp/canvas) in deps and it matches the user's "paste-immediately" mental model. If a target app does NOT stack multiple pasted images, build a montage (combine tray into one numbered PNG) — that needs a renderer canvas or an offscreen window since the main process has no canvas.

**Phase 2 (built): freeze-and-drag annotation**
- Hotkey slot `"drag"` (default `CommandOrControl+Shift+1`, persisted via `environment.js` `getDragKey/saveDragKey` → `DRAG_KEY`, rebindable via `register-drag-hotkey` IPC). NOTE: do NOT default to `Cmd+Shift+3/4/5` — those are reserved macOS screenshot shortcuts the OS intercepts before Electron sees them (this bit us once). Region-screenshot slot default is `Cmd+Shift+2`. Registered in `main.js` next to the screenshot slot. The callback toggles: if the overlay is visible → `windowManager.finishAnnotation()` (commit); otherwise `captureTargetPid()` + `screenshotManager.captureCursorDisplay()` + `windowManager.showFreezeOverlay(snapshot)`.
- `captureCursorDisplay()` snapshots the whole cursor display FIRST (`screencapture -x -D<id>`), so an open right-click menu stays in the snapshot even after the overlay takes focus and the real menu dismisses. Returns `{ dataUrl, display: { id, bounds, scaleFactor } }`.
- Overlay window: `FREEZE_OVERLAY_CONFIG` in `windowConfig.js` (frameless, `transparent:true` to avoid a flash before paint, `focusable:true`, no `setIgnoreMouseEvents` so it captures all input → the background is effectively frozen). Created hidden at startup via `windowManager.createAnnotationWindow()`; `showFreezeOverlay()` sets bounds to the cursor display, raises z-level to `"screen-saver"` (above menubar), and sends `annotation-init` with the snapshot.
- Renderer view `src/components/AnnotationOverlay.tsx` routed via `annotate=true` in `AppRouter.jsx`. Canvas backs at device pixels (`innerWidth * devicePixelRatio`) and draws in CSS px via `ctx.setTransform(dpr,...)`, so the exported PNG is full Retina resolution. Draws the frozen snapshot + arrows (line + `atan2` arrowhead) + a custom coloured cursor ring (system cursor hidden with `cursor:none`). Mouse drag = one arrow; multiple arrows supported; small toolbar (colour swatches, Undo, Annuleren, Klaar). Keys: Esc cancel, Enter/`annotation-finish` commit, Cmd/Ctrl+Z undo.
- On commit: `canvas.toDataURL("image/png")` (cursor excluded from export) → `annotation-done` IPC → `screenshotManager.addDataUrl()` → the frame joins the same tray and flushes with the dictation. `annotation-cancel` just hides the overlay. Preload: `registerDragHotkey`, `annotationDone`, `annotationCancel`, `onAnnotationInit`, `onAnnotationFinish`, `onAnnotationReset`.
- **Known risks to watch on real-app testing**: overlay z-level above the menubar/fullscreen apps; firing the snapshot in the same tick as the hotkey so an open menu is captured; sequential image+text paste in Chromium editors (Cursor). electron.ts types for the new electronAPI methods are not yet added (renderer uses an `any` cast); add them when a settings UI is built.
